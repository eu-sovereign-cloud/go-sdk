...
func TestListInstancesWithFiltersV1(t *testing.T) {
	...
	skuRef := &schema.Reference{Resource: secatest.InstanceSku1Ref}
...
func TestGetInstanceUntilNotFoundV1(t *testing.T) {
	ctx := context.Background()
	sm := http.NewServeMux()
	...
	err := regionalClient.ComputeV1.GetInstanceUntilNotFound(ctx, wref, config)
	assert.NoError(t, err)
}
...
func buildResponseInstanceSpec(skuRef string, zone string) *schema.InstanceSpec {
